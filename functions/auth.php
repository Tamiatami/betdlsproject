...
function auth_authByLogin($data, $callback){
	...
	if(!$row2) return $callback('User logged in with Auth unsuccessfully (1)', null); //ERROR
	if($sql2->rowCount() <= 0) return $callback('User logged in with Auth unsuccessfully (2)', null); //ERROR
	
	return auth_checkSession($row1['userid'], $callback);
}

function auth_referral($data){
	$sql1 = $GLOBALS['db']->query('SELECT * FROM `referral_uses` WHERE `userid` = ' . $GLOBALS['db']->quote($data['userid']));
	
	if($sql1->rowCount() <= 0) {
		$sql2 = $GLOBALS['db']->query('SELECT * FROM `referral_codes` WHERE `code` = ' . $GLOBALS['db']->quote($data['code']));
		
		if($sql2->rowCount() > 0) {
			$row2 = $sql2->fetch(PDO::FETCH_ASSOC);
			
			if($row2['userid'] != $data['userid']){
				$amount_got = getFormatAmount($GLOBALS['rewards_amounts']['refferal_code']);
				
				$sql3 = $GLOBALS['db']->prepare('INSERT INTO `referral_uses` SET `userid` = ' . $GLOBALS['db']->quote($data['userid']) . ', `referral` = ' . $GLOBALS['db']->quote($row2['userid']) . ', `amount` = ' . $GLOBALS['db']->quote($amount_got) . ', `time` = ' . $GLOBALS['db']->quote(time()));
				$row3 = $sql3->execute();
				
				if($row3 && $sql3->rowCount() > 0){
					$sql4 = $GLOBALS['db']->prepare('INSERT INTO `users_rewards` SET `userid` = ' . $GLOBALS['db']->quote($data['userid']) . ', `reward` = ' . $GLOBALS['db']->quote("referral") . ', `amount` = ' . $GLOBALS['db']->quote($amount_got) . ', `time` = ' . $GLOBALS['db']->quote(time()));
					$row4 = $sql4->execute();
					
					if($row4 && $sql4->rowCount() > 0){
						$sql5 = $GLOBALS['db']->prepare('UPDATE `users` SET `balance` = `balance` + ' . $GLOBALS['db']->quote($amount_got) . ' WHERE `userid` = ' . $GLOBALS['db']->quote($data['userid']));
						$row5 = $sql5->execute();
						
						if($row5 && $sql5->rowCount() > 0){
							$sql6 = $GLOBALS['db']->prepare('INSERT INTO `users_transactions` SET `userid` = ' . $GLOBALS['db']->quote($data['userid']) . ', `service` = ' . $GLOBALS['db']->quote("code_reward") . ', `amount` = ' . $GLOBALS['db']->quote($amount_got) . ', `time` = ' . $GLOBALS['db']->quote(time()));
							$row6 = $sql6->execute();
							
							if($row6 && $sql6->rowCount() > 0){
...
function auth_authByRegister($data, $callback){
	...
	$name = $data['username'];
	$avatar = 'https://betdls.com/random_pictures/' . rand(1, 7) . '.png';
	...
	if(!empty($data['referral'])) auth_referral(array( 'userid' => $userid, 'code' => $data['referral'] ));
...
function auth_authBySteam($data, $callback){
	...
	if($sql1->rowCount() > 0) {
		...
		$sql5 = $GLOBALS['db']->prepare('INSERT INTO `users_seeds_client` SET `userid` = ' . $GLOBALS['db']->quote($userid) . ', `seed` = ' . $GLOBALS['db']->quote($client_seed) . ', `time` = ' . $GLOBALS['db']->quote(time()));
		$row5 = $sql5->execute();
		
		$sql6 = $GLOBALS['db']->prepare('INSERT INTO `users_seeds_server` SET `userid` = ' . $GLOBALS['db']->quote($userid) . ', `seed` = ' . $GLOBALS['db']->quote($server_seed) . ', `time` = ' . $GLOBALS['db']->quote(time()));
		$row6 = $sql6->execute();
...
function auth_authByGoogle($data, $callback){
	...
	if($sql1->rowCount() > 0) {
		...
		if($sql2->rowCount() > 0) {
			...
			$sql3 = $GLOBALS['db']->prepare('INSERT INTO `users_binds` SET `userid` = ' . $GLOBALS['db']->quote($row2['userid']) . ', `bind` = ' . $GLOBALS['db']->quote('google') . ', `bindid` = ' . $GLOBALS['db']->quote($data['id']) . ', `time` = ' . $GLOBALS['db']->quote(time()));
			$row3 = $sql3->execute();
			
			if(!$row3) return $callback('User logged in and binded with Google unsuccessfully (1)', null); //ERROR
			if($sql3->rowCount() <= 0) return $callback('User logged in  and binded with Google unsuccessfully (2)', null); //ERROR
			...
			if(!$row4) return $callback('User logged in and binded with Google unsuccessfully (3)', null); //ERROR
			if($sql4->rowCount() <= 0) return $callback('User logged in and binded with Google unsuccessfully (4)', null); //ERROR
			...
			$sql6 = $GLOBALS['db']->prepare('INSERT INTO `users_seeds_client` SET `userid` = ' . $GLOBALS['db']->quote($userid) . ', `seed` = ' . $GLOBALS['db']->quote($client_seed) . ', `time` = ' . $GLOBALS['db']->quote(time()));
			$row6 = $sql6->execute();
			
			$sql7 = $GLOBALS['db']->prepare('INSERT INTO `users_seeds_server` SET `userid` = ' . $GLOBALS['db']->quote($userid) . ', `seed` = ' . $GLOBALS['db']->quote($server_seed) . ', `time` = ' . $GLOBALS['db']->quote(time()));
			$row7 = $sql7->execute();
...
function auth_insertSession($userid, $callback) {
	...
	$sql1 = $GLOBALS['db']->prepare('INSERT INTO `users_sessions` SET `userid` = ' . $GLOBALS['db']->quote($userid) . ', `activated` = 1, `session` = ' . $GLOBALS['db']->quote($session) . ', `expire` = ' . $GLOBALS['db']->quote($expire) . ', `created` = ' . $GLOBALS['db']->quote(time()));
	$row1 = $sql1->execute();
	
	if(!$row1) return $callback('Session registered unsuccessfully (1)', null); //ERROR
	if($sql1->rowCount() <= 0) return $callback('Session registered unsuccessfully (2)', null); //ERROR
